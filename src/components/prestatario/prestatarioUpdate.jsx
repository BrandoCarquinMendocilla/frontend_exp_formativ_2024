...
const FormUpdatePrestatario = () => {
    ...
    useEffect(() => {
        const getUserById = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/v1/private/prestatario/${id}`);
                const body = response.data.body
                ...
                setRecomendacion(user.recomendacion);
    ...
    const updateJefePrestamista = async (e) => {
        ...
        validateDocumento();
        if (nameError || apellidoError || direccionError  || telefonoError || tipoDocumentoError || documentoError) {
            Swal.fire({
                icon: "warning",
                title: "Error",
            ...
            try {
                ...
                if (validate) {
                    await axios.patch("http://localhost:3000/v1/private/update/prestatario/" + id, {
                        name,
                        apellido,
                        direccion,
                        telefono,
        ...
        <div className="container mt-4">
            <div className="card">
                <header className="card-header">
                    <p className="card-header-title">EDITAR PRESTATARIO : {name} </p>
                </header>
...
export default FormUpdatePrestatario;
