...
export const PrestamistaList = () => {
    ...
    const getListPrestamista = async () => {
        ...
        setPrestamista(body.data.users);
    ...
    const deleteJefeProduct = async (userId, email) => {
        ...
        setEvent('DELETE_PRESTAMISTA')
        await sendOtpEmail('DELETE_PRESTAMISTA', user.correo)
        setOtpCode('')
        setErrorOtp('')
        setShowOtpForm(true);
    }
    ...
    const updatePrestamista = async (userId, correo) => {
        ...
        setOtpCode('')
        setErrorOtp('')
        setEvent('CHANGE_PASSWORD_PRESTAMISTA')
        await sendOtpEmail('CHANGE_PASSWORD_PRESTAMISTA', user.correo)
        setConfirmPassword('')
        setNewPassword('')
        setPasswordError('')
        setShowOtpForm(true);
    ...
    const validateOtpCode = async () => {
        ...
        if (valdiate) {
            ...
            if (event === 'DELETE_PRESTAMISTA') {
                setShowDeletePrestamista(true);
            }
            if (event === 'CHANGE_PASSWORD_PRESTAMISTA') {
        ...
        <div className="container mt-4">
            <div className="card">
                <header className="card-header">
                    <p className="card-header-title">LISTA DE PRESTAMISTAS</p>
                </header>
                <div className="card-content">
                    ...
                    <div className="content">
                        ...
                        <div style={{ textAlign: 'right', marginBottom: '1rem' }}> {/* Estilo inline para alinear a la derecha */}
                            <Link to="/prestamistas/add" className="button is-primary" style={{ backgroundColor: 'black', color: 'white' }}>
                                Añadir Nuevo
                            </Link>
                        </div>
                        {loading ? (
                                    ...
                                    <tbody>
                                        {currentItems.map((user, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <Link
                                                        to={`/prestamistas/edit/${user.userId}`}
