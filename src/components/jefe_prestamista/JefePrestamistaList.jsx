...
export const JefePrestamistaList = () => {
    const [jefePrestamista, setJefePrestamista] = useState([]);
    ...
    const [showDeleteJefePrestamista, setShowDeleteJefePrestamista] = useState(false);
    const [deleteEmailJP, setDeleteEmailJP] = useState('');
    const [deleteUserIdJP, setDeleteUserIdJP] = useState('');
    const [event, setEvent] = useState('');

    const deleteJefeProduct = async (userId, email) => {
        setDeleteEmailJP(email);
        setDeleteUserIdJP(userId);
        setEvent('DELETE_JEFE_PRESTAMISTA')
        await sendOtpEmail('DELETE_JEFE_PRESTAMISTA', user.correo)
        setOtpCode('')
        setErrorOtp('')
        setShowOtpForm(true);
    }
    ...
    const deleteJefePrestamista = async () => {
        ...
        Swal.fire({
            ...
            html: `<div style="font-size: 16px; font-weight: normal;">Se elimino correctamente el usuario ${deleteEmailJP}</div>`
        });
        getListJefePrestamista();
        setShowDeleteJefePrestamista(false)
    }

    const [updateEmailJP, setUpdateEmailJP] = useState('');
    const [updateUserIdJP, setUpdateUserIdJP] = useState('');

    const updateJefePrestamista = async (userId, correo) => {
        setUpdateEmailJP(correo);
        setUpdateUserIdJP(userId);
        setOtpCode('')
        setErrorOtp('')
        setEvent('CHANGE_PASSWORD_JEFE_PRESTAMISTA')
        await sendOtpEmail('CHANGE_PASSWORD_JEFE_PRESTAMISTA', user.correo)
        setConfirmPassword('')
        setNewPassword('')
        setPasswordError('')
        setShowOtpForm(true);
    ...
    const validateOtpCode = async () => {
        ...
        if (valdiate) {
            ...
            if (event === 'DELETE_JEFE_PRESTAMISTA') {
                setShowDeleteJefePrestamista(true);
            }
            if (event === 'CHANGE_PASSWORD_JEFE_PRESTAMISTA') {
    ...
    const currentItems = jefePrestamista.slice(indexOfFirstItem, indexOfLastItem);
        ...
        <div className="container mt-4">
            <div className="card">
                <header className="card-header">
                    <p className="card-header-title">LISTA DE JEFE DE PRESTAMISTAS</p>
                </header>
                <div className="card-content">
                    ...
                    <div className="content">
                        ...
                        <div style={{ textAlign: 'right', marginBottom: '1rem' }}> {/* Estilo inline para alinear a la derecha */}
                            <Link to="/jefe_prestamistas/add" className="button is-primary" style={{ backgroundColor: 'black', color: 'white' }}>
                                Añadir Nuevo
                            </Link>
                        </div>
                        {loading ? (
                                    ...
                                    <tbody>
                                        {currentItems.map((user, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <Link
                                                        to={`/jefe_prestamistas/edit/${user.userId}`}
                                                    ...
                                                    <button
                                                        onClick={() => updateJefePrestamista(user.userId, user.correo)}
                                                        className="button is-small is-info mr-1"
                            ...
                            <div className="modal is-active">
                                ...
                                <div className="modal-card">
                                    ...
                                    <section className="modal-card-body" style={{ borderRadius: '0 0 10px 10px' }}>
                                        <p>Usuario a eliminar {deleteEmailJP}</p>
                                        <br />
